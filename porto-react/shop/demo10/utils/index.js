...
export function stickyInit () {
    ...
    let stickyHeader3 = document.querySelector( '.main .sticky-header.sticky-cart' + stickyType );
    if ( stickyHeader3 ) {
        let height = parseInt( stickyHeader3.offsetHeight ) + parseInt( window.getComputedStyle( stickyHeader3 ).getPropertyValue( 'margin-bottom' ).slice( 0, -2 ) );

        if ( window.innerWidth < 992 && stickyHeader3.classList.contains( 'mobile-sticky' ) || window.innerWidth >= 992 && stickyHeader3.classList.contains( 'desktop-sticky' ) ) {
            if ( stickyHeader1 ) {
                if ( stickyHeader2 ) {
                    stickyHeaderHandler( stickyHeader3, stickyHeader2.offsetHeight, height );
                } else {
                    stickyHeaderHandler( stickyHeader3, stickyHeader1.offsetHeight, height );
                }
            } else {
                stickyHeaderHandler( stickyHeader3, 0, height );
            }
        }
    }
