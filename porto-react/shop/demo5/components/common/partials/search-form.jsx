...
function SearchForm(props) {
    ...
    return (
        <div className={`header-icon header-search header-search-inline header-search-category w-lg-max ${nogap ? '' : 'ml-3 mr-xl-4'} `}>
                    ...
                    <div className="live-search-list bg-white">
                        {search.length > 2 && data && data.products.data.map((product, index) => (
                            <ALink href={`/product/default/${product.slug}`} className="autocomplete-suggestion" key={`search-result-${index}`}>
                                <LazyLoadImage src={process.env.NEXT_PUBLIC_SERVER_URL + product.small_pictures[0].url} width={40} height={40} alt="product" />
