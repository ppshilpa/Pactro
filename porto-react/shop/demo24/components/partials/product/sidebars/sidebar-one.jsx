...
function ProductSidebarOne ( props ) {
    ...
    return (
        <>
            ...
            <aside className={ `sidebar-product col-lg-3 pt-5 mobile-sidebar skeleton-body skel-shop-products ${ adClass } ${ loading ? '' : 'loaded' }` }>
                <StickyBox className="sticky-wrapper pt-2" offsetTop={ 70 }>
                    {
                        loading ?
                                            ...
                                            <div className="overflow-hidden" ref={ setCollapsibleElement }>
                                                <div className="widget-body">
                                                    <ul className="cat-list">
                                                        {
                                                            data && data.shopSidebarData.categories.map( ( cat, index ) => (
                                                                <li key={ `sidebar-category-${ index }` }>
                                                                    <ALink
                                                                        href={ { pathname: '/shop', query: { category: cat.slug } } }
                                                                        scroll={ false }
                                                                    >{ cat.name } <span className="products-count">({ cat.count })</span></ALink></li>
                                                            ) )
                                                        }
                                                    </ul>
                                                </div>
