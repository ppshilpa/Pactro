        ...
        <>
            ...
            <aside className={`toolbox-left sidebar-shop mobile-sidebar ${adClass}`}>
                <div className="toolbox-item toolbox-sort select-custom" onClick={openList}>
                    <span className="sort-menu-trigger">Categories</span>
                    <ul className="sort-list">
                        {
                            data && data.shopSidebarData.categories.map((cat, index) => (
                                <li key={`sidebar-category-${index}`} className={`${query.category === cat.slug ? 'active' : ''}`}>
                                    <ALink
                                        href={{ query: { category: cat.slug, grid: query.grid } }}
                                        scroll={false}
                                    >{cat.name} <span className="products-count0">({cat.count})</span></ALink></li>
                            ))
                        }
                    </ul>
                </div>
