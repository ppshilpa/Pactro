                            ...
                            <li className={ pathname.startsWith( '/product' ) ? 'active' : '' }>
                                {
                                    <SlideToggle collapsed={ true }>
                                        { ( { onToggle, setCollapsibleElement, toggleState } ) => (
                                            <>
                                                <ALink href="/product/right-sidebar/jordan-flight" className={ `${ toggleState.toLowerCase() }` }>Products<span className="mmenu-btn" onClick={ e => { e.preventDefault(); onToggle(); } }></span></ALink>
