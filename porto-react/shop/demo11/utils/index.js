...
export function setParallax() {
    ...
    if (parallax) {
        let y = (parallax.getBoundingClientRect().top) / 20 + 40;
...
// controll word-rotating
export const wordRotate = function () {
    let element = document.querySelector('.wort-rotator');
    let interval;
    let offset = 0;
    let rotateDelay = 1000;
    let height = 1;
    let counts = 0;
    let item;
    ...
    function core() {
        offset -= height;
        item.setAttribute('style', `top: ${offset}px`);

        setTimeout(() => {
            if (height * counts + offset <= 0) offset = 0;
            item.setAttribute('style', `transition: none; top: ${offset}px`);
        }, 300);
    }
    ...
    function rotateHandler() {
        ...
        rotateDelay = parseInt(parseOptions(element.getAttribute('data-plugin-options')).delay);
        let content = element.querySelector('.wort-rotator-items span:last-child');
        counts = document.querySelectorAll('.wort-rotator-items span').length;
        element.setAttribute('style', `height: ${content.offsetHeight}px`);
        element.classList.add('active');
        item = element.querySelector('.wort-rotator-items');
        var d1 = content;
        var d2 = document.querySelector('.wort-rotator-items span');
        d1.insertAdjacentHTML('afterend', d2.outerHTML);
        height = content.offsetHeight;
        interval = setInterval(core, rotateDelay);
    }
