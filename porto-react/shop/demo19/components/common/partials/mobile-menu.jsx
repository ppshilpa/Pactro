                            ...
                            <li className={ pathname.startsWith( '/product' ) ? 'active' : '' }>
                                {
                                    <SlideToggle collapsed={ true }>
                                        { ( { onToggle, setCollapsibleElement, toggleState } ) => (
                                            <>
                                                <ALink href="/product/right-sidebar/avengers-2-age-of-ultron" className={ `${ toggleState.toLowerCase() }` }>Products<span className="mmenu-btn" onClick={ e => { e.preventDefault(); onToggle(); } }></span></ALink>
